...
DragGame::DragGame(sf::RenderWindow& window, std::vector<Image>& dragPic,
    std::vector<Image>& dropPic, const std::string& background) : Window(window, background),
    dropPic(dropPic), dragPic(dragPic)
{
    ...
    setSymetric(Singleton::get().figures, 0, width, dragPic[0].getShape().getSize().x, dragY);
    setSymetric(Singleton::get().figuresPlace, 0, width, dragPic[0].getShape().getSize().x, dropY);
}
